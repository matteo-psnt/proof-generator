...
export function validateVariableCount(expression: BooleanExpression): {
  ...
  if (variableCount > MAX_TRUTH_TABLE_VARIABLES) {
    return {
      ...
      variableCount,
      error:
        `Too many variables (${variableCount}). Truth tables are limited to ${MAX_TRUTH_TABLE_VARIABLES} variables maximum ` +
...
export function generateTruthTable(expression: BooleanExpression): TruthTable {
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    try {
      ...
      console.error(
        `Error evaluating expression with assignment ${JSON.stringify(assignment)}:`,
        error
      );
...
export function formatTruthTable(
  ...
  const { useSymbols = true, includeExpression = true, separator = ' | ' } = options;
...
export function truthTableToCSV(truthTable: TruthTable): string {
  ...
  for (const row of truthTable.rows) {
    const values = truthTable.variables.map((variable) => (row.assignment[variable] ? '1' : '0'));
...
export function findSatisfyingAssignments(expression: BooleanExpression): Assignment[] {
  ...
  return truthTable.rows.filter((row) => row.result).map((row) => row.assignment);
}
