    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
          {variant === 'ast' && (
            <svg className="w-5 h-5 mr-2 text-gray-600" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
                d="M3 4a1 1 0 011-1h4a1 1 0 010 2H6.414l2.293 2.293a1 1 0 01.293.707v2.586l2.293 2.293A1 1 0 0112 13h4a1 1 0 110 2h-4a1 1 0 01-.707-.293L9 12.414V9.828L6.707 7.535A1 1 0 016 7H4a1 1 0 01-1-1V4z"
                clipRule="evenodd"
              />
            </svg>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-3">
            <button onClick={handleClear} className="btn-secondary" disabled={!inputValue}>
              <Trash2 className="h-4 w-4 mr-2" />
              Clear
            </button>

            <button onClick={handleCopy} className="btn-outline" disabled={!currentExpression}>
      ...
      {currentExpression && (
        <div className="card">
          ...
          <div className="expression">{currentExpression.toString()}</div>

          <div className="mt-4 grid grid-cols-2 gap-4 text-sm text-gray-600">
            <div>
              <span className="font-medium">Variables:</span>{' '}
              {Array.from(currentExpression.getVariables()).join(', ') || 'None'}
            </div>
            <div>
      ...
      {showSyntaxHelp && (
        <div className="card bg-blue-50 border-blue-200">
          ...
          <div className="space-y-2 text-sm text-blue-800">
            <div>
              <strong>AND:</strong> &, ∧, ^, &&, AND, and, *, .
            </div>
            <div>
              <strong>OR:</strong> |, ∨, v, ||, OR, or, +
            </div>
            <div>
              <strong>NOT:</strong> !, ¬, ~, NOT, not
            </div>
            <div>
              <strong>IMPLIES:</strong> {'=>'}, →, {'->'}, IMP, imp, implies
            </div>
            <div>
              <strong>IFF:</strong> {'<=>'}, ↔, {'<->'}, IFF, iff, equiv
            </div>
            <div>
              <strong>Constants:</strong> true, false, TRUE, True, FALSE, False, T, t, F, f, 1, 0
            </div>
            <div>
              <strong>Variables:</strong> Any alphanumeric identifier (a, b, P, Q, var1, etc.)
            </div>
            <div>
              <strong>Grouping:</strong> Use parentheses ( ) to control precedence
            </div>
          </div>
        </div>
      )}
